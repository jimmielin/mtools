...
tmp2D_Bdg_NH3 = zeros(length(lons), length(lats), times);
tmp2D_Bdg_NO  = zeros(length(lons), length(lats), times);
tmp2D_Bdg_NO2 = zeros(length(lons), length(lats), times);
tmp2D_Bdg_SO2 = zeros(length(lons), length(lats), times);
...
for t = 1:length(date_strGC)
    ...
    tmp4D_read = ncread(fname, 'BudgetEmisDryDepFull_NH3');
    tmp2D_Bdg_NH3(:,:,t) = tmp4D_read(:,:,1,1); % lev, time collapsed
    
    tmp4D_read = ncread(fname, 'BudgetEmisDryDepFull_NO2');
    tmp2D_Bdg_NO(:,:,t) = tmp4D_read(:,:,1,1); % lev, time collapsed
    
    tmp4D_read = ncread(fname, 'BudgetEmisDryDepFull_NO2');
    tmp2D_Bdg_NO2(:,:,t) = tmp4D_read(:,:,1,1); % lev, time collapsed
    
    tmp4D_read = ncread(fname, 'BudgetEmisDryDepFull_SO2');
    tmp2D_Bdg_SO2(:,:,t) = tmp4D_read(:,:,1,1); % lev, time collapsed
end

save('tmp_budget_2.mat', 'lats', 'lons', 'numtimes', ...
     'tmp2D_Bdg_NH3', 'tmp2D_Bdg_NO2', 'tmp2D_Bdg_NO', 'tmp2D_Bdg_SO2');